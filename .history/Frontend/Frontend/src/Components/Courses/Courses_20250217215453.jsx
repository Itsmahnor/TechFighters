...
const CategorySlider = ({ settings }) => (
  <Slider {...settings} className="flex gap-4">
    {Course.map((item, index) => (
      <div key={index} className="w-full h-auto bg-[#B185DB] rounded-3xl">
        <div className="md:m-4 m-2">
          <img
            src={item.img}
            alt={item.title}
            className="h-[350px] object-contain rounded-2xl filter brightness-75 transition duration-300 hover:brightness-50 cursor-pointer"
          />
